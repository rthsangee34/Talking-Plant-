...
import { Sprout, X, Shield, Loader2, Download, AlertCircle, CheckCircle2 } from 'lucide-react';
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (user) {
        ...
        // If popup was blocked or closed, give clear guidance
        setError(authError.includes('popup-closed-by-user')
    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          {/* Modal Panel: opacity 0 -> 1, scale 0.96 -> 1 */}
          <motion.div
            initial={{ opacity: 0, scale: 0.96, y: 10 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            exit={{ opacity: 0, scale: 0.96, y: 10 }}
            transition={{ duration: 0.3, ease: [0.16, 1, 0.3, 1] }}
            className="relative w-full max-w-md bg-white/95 backdrop-blur-2xl rounded-3xl p-6 sm:p-8 shadow-2xl border border-white/80 text-stone-900 overflow-hidden"
          >
            ...
            <div className="flex flex-col items-center text-center mb-6">
              ...
              <h3 className="text-2xl font-bold text-emerald-950 tracking-tight font-['Outfit']">
                Install & Open Plant Talk
              </h3>
            ...
            <div className="space-y-3">
              ...
              {deferredPrompt && (
                <button
                  ...
                  className="w-full py-3 px-4 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-900 border border-emerald-200 text-xs font-bold flex items-center justify-center gap-2 transition-all cursor-pointer active:scale-98"
                >
            ...
            <div className="mt-5 pt-4 border-t border-stone-200/80 flex items-center justify-center gap-2 text-[11px] text-stone-500">
